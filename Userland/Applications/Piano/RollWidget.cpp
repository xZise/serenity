...
void RollWidget::mousemove_event(GUI::MouseEvent& event)
{
    ...
    if (m_mousedown_event.value().button() == GUI::MouseButton::Primary) {
        int const x0 = get_note_for_x(m_mousedown_event.value().x());
        int const x1 = get_note_for_x(event.x());

        const u32 on_sample = roll_length * (static_cast<double>(min(x0, x1)) / m_num_notes);
        const u32 off_sample = (roll_length * (static_cast<double>(max(x0, x1) + 1) / m_num_notes)) - 1;
        auto const note = RollNote { on_sample, off_sample, get_pitch_for_y(event.y()), 127 };

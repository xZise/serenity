...
Audio::Sample WaveWidget::getMax(FixedArray<Audio::Sample> const& buffer)
{
    Audio::Sample result(NumericLimits<float>::min(), NumericLimits<float>::min());
    for (size_t x = 0; x < buffer.size(); x++) {
        result.left = max(result.left, absf(buffer[x].left));
        result.right = max(result.right, absf(buffer[x].right));
    }
    return result;
}
...
void WaveWidget::paint_event(GUI::PaintEvent& event)
{
    ...
    m_track_manager.current_track()->current_signal(buffer);
    double width_scale = static_cast<double>(frame_inner_rect().width()) / buffer.size();

    auto const maximum = getMax(buffer);
    int prev_x = 0;
